...
| 切块容器跟随源音频扩展名（不再固定 `.m4a`） | 2026-09-18（L015 后修复） | design 的数据流/关键节点写 `chunks/*.m4a`；切块用 `-c copy`，容器必须匹配编码：yt-dlp 常给 webm/opus，写 mp4/ipod 容器会 `Could not find tag for codec opus in stream #0`（exit -22） | 接受：`chunk_file_name` 复用源文件扩展名（无扩展名回退 `m4a`）；whisper 的 JSON 路径由 file_stem 推导，不受影响 |

...
Requirements: Node.js (v24+), Rust (CI pins `1.94.1`; latest stable works locally) and the platform's [Tauri system dependencies](https://tauri.app/start/prerequisites/).

```shell
npm install                 # install frontend dependencies
npm run tauri dev           # run the full app (builds the isolation page, then starts Vite + Rust)
npm run build               # frontend only: app bundle (dist/) + isolation bundle (dist-isolation/)
npm run tauri build         # build installers/bundles for the current platform
```

`npm run tauri build` follows `src-tauri/tauri.conf.json`:

| Platform | Artifacts |
| --- | --- |
| Windows | NSIS installer: `Open.Video.Downloader_<version>_x64-setup.exe` (also arm64) |
| macOS | `.app` and `.dmg` for x64 and aarch64 |
| Linux | `AppImage`, `.deb`, `.rpm` (x64 and aarch64) |

Things to keep in mind when packaging:

- The version number lives in three files and must stay in sync: `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`.
- Updater artifacts are always generated (`createUpdaterArtifacts: true`); the in-app updater reads `plugins.updater.endpoints` (GitHub Releases `latest.json`) and verifies its signature.
- yt-dlp and ffmpeg are **not** bundled. They are installed at first launch from the signed manifest hosted on GitHub Pages, generated by `npm run manifest:gen` / `manifest:sign` / `manifest:verify` (`.github/workflows/pages.yml`).
- Real releases are built in CI, not locally: `.github/workflows/publish.yml` builds six targets (macOS x64/arm64, Ubuntu x64/arm64, Windows x64/arm64) and publishes the GitHub Release; `distribute.yml` submits winget/Snap, `msix.yml` handles the Microsoft Store.
- Common failures: `npm run build:isolation` must succeed before dev/packaging (a missing `dist-isolation` breaks Tauri builds); Linux builds need `glib-2.0`/`webkit2gtk` packages; Windows/macOS code signing needs the secrets listed in `docs/current/platform/env.md`.

Deeper reference (Chinese docs): `docs/current/platform/build-test-lint.md` (commands, tests, CI) and `docs/current/platform/release-and-distribution.md` (release pipeline, signing manifest, distribution channels).
